...
- **Flexible Content Management**: Load content from any source (files, APIs, CMSs) using loaders
...
- **Pluggable Loaders**: Use built-in loaders or create custom ones
...
**Note**: Content Layer uses loaders to fetch data. For local files, use the `glob` loader. For remote sources, use community loaders or create custom ones.
...
- **Fallback Content**: Display placeholder content while islands load
...
Server islands use the `server:defer` directive (not `prerender = false`). They run on the server and are fetched independently after the initial page load.
...
- **Virtual Module**: Access via `astro:env/client` and `astro:env/server` imports
...
**Note**: `astro:env` is a virtual module and can only be used in Astro contexts (components, routes, endpoints, middleware). It cannot be used in `astro.config.mjs` or scripts.
...
**Note**: Container API is experimental and subject to breaking changes. Use `experimental_AstroContainer` from `astro/container`.
...
**Important**: The Container API works in Vite environments (like Vitest). For on-demand rendering or non-Vite environments, renderers must be manually imported.
...
// Recommendations component will be deferred
---
...
// This component runs on the server when the island loads
...
```typescript
import { PUBLIC_SITE_URL, PUBLIC_ANALYTICS_ID } from "astro:env/client";
import { DATABASE_URL, API_SECRET_KEY } from "astro:env/server";
```
